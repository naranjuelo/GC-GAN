...
r_eye_ids = (246, 161, 160, 159, 158, 157, 173, 33, 7, 163, 144, 145, 153, 154, 155, 133)#, 247, 30, 29, 27, 28, 56, 190, 130, 25, 110, 24, 23, 22, 26, 112, 243,  113, 225, 224, 223, 222, 221, 189, 226, 31, 228, 229, 230, 231, 232, 233, 244, 143, 111, 117, 118, 119, 120, 121, 128, 245)
...
def __get_face_parts(face_landmarks: landmark_pb2.NormalizedLandmarkList,
                              image_cols: int, image_rows: int):
    r_eye_landmarks = []
    l_eye_landmarks = []
    mouth_landmarks = []
    ...
    return l_eye_landmarks, r_eye_landmarks, mouth_landmarks, l_eyebrow_landmarks, r_eyebrow_landmarks, nose_landmarks

def __get_face_parts_all(face_landmarks: landmark_pb2.NormalizedLandmarkList,
                              image_cols: int, image_rows: int):
    r_eye_landmarks = []
    l_eye_landmarks = []
    mouth_landmarks = []
    ...
    return l_eye_landmarks, r_eye_landmarks, mouth_landmarks, l_eyebrow_landmarks, r_eyebrow_landmarks, nose_landmarks, all_landmarks
...
def __crops_face_AFFNet(img, l_center, r_center, mouth_lnds, out_size, w, h):
        ...
        if 0 == n_mtl:
            ...
            avg_mouth_y = avg_eyes_y # avg_eyes_y + (r_center[0] + l_center[0])
        else:
        ...
        #cv2.imwrite('tmpcrop.jpg', cropped_img_face)
        ...
        x1 = min(w, xf1)
        c = img.copy()
        #cv2.imwrite('tmpim.jpg', img)
        eyes_crop = img[y0:y1, x0:x1]
        #cv2.imwrite('tmp.jpg', eyes_crop)
        #cropped = face[0:112, 0:224]
...
# For static images:
IMAGE_FILES = ['C:/Users/naranjuelo/Desktop/tmp/tmp.jpg']
