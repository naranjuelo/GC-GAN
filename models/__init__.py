...
# 25/05/2024: add make_model_gaze 

import pprint
from .semantic_stylegan import SemanticGenerator, SemanticGenerator_gaze, DualBranchDiscriminator, DualBranchDiscriminator_gaze   
...
def make_model(args, verbose=True):
    ...
    model = SemanticGenerator(args.size, args.latent, args.n_mlp, 
        ...
        estim_gaze=args.gaze_regression)
    return model

def make_model_gaze(args, verbose=True):
    if verbose:
        print(f"Initializing model with arguments:")
        pprint.pprint(vars(args))
    model = SemanticGenerator_gaze(args.size, args.latent, args.n_mlp, 
        channel_multiplier=args.channel_multiplier, seg_dim=args.seg_dim, 
        local_layers=args.local_layers, local_channel=args.local_channel,
        base_layers=args.base_layers, depth_layers=args.depth_layers,
        coarse_size=args.coarse_size, coarse_channel=args.coarse_channel, min_feat_size=args.min_feat_size, 
...
# def make_model_face(args, verbose=True):
#     if verbose:
#         print(f"Initializing model with arguments:")
#         pprint.pprint(vars(args))
#     model = SemanticGenerator_in_efeats(args.size, args.latent, args.n_mlp, 
#         channel_multiplier=args.channel_multiplier, seg_dim=args.seg_dim, 
#         local_layers=args.local_layers, local_channel=args.local_channel,
#         base_layers=args.base_layers, depth_layers=args.depth_layers,
#         coarse_size=args.coarse_size, coarse_channel=args.coarse_channel, min_feat_size=args.min_feat_size, 
...
# def make_model_composition(args, verbose=True):
#     if verbose:
#         print(f"Initializing model with args")
#         pprint.pprint(vars(args))
#     model = SemanticCompose_unet(args.size)
#     return model
