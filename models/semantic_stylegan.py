...
# 25/05/2024: add localGenerator_gaze, RenderNet_gaze, 
# SemanticGenerator_gaze, DualBranchDiscriminator_gaze
...
class LocalGenerator_gaze(nn.Module):
    def __init__(self, in_channel, out_channel, hidden_channel, style_dim, n_layers=8, depth_layers=8, use_depth=False, detach_texture=False):
        super().__init__()
        self.n_layers = n_layers
        self.depth_layers = depth_layers
        ...
        self.detach_texture = detach_texture
        self.linears = nn.ModuleList()
        self.embed_features = 64
        style_dim = style_dim + self.embed_features
        for _ in range(n_layers):
            self.linears.append(StyledConv(in_channel, hidden_channel, 1, style_dim, inject_noise=False))
            in_channel = hidden_channel
        self.to_feat = ToRGB(hidden_channel, out_channel, style_dim)
        if self.use_depth:
            self.to_depth = ToRGB(hidden_channel, 1, style_dim)
        c_dim = 2
        self.embed = FullyConnectedLayer(c_dim, self.embed_features)
        #self.embed_g_l = FullyConnectedLayer(5760, 5120)

    def forward(self, x, latent, gaze):
        depth = torch.zeros(x.size(0), 1, x.size(2), x.size(3)).to(x.device)

        gaze_ext = (self.embed(gaze.to(torch.float32)))
        gaze_ext = gaze_ext.unsqueeze(1).repeat(1, 8, 1) # only for texture, shape latents
        dummy = torch.zeros(x.size(0), 2, self.embed_features).to(x.device) # fill until 10 with 0
        gaze_ext = torch.cat([dummy, gaze_ext], dim=1) 
        #print(gaze_ext.size())
        latent_ext = torch.cat([latent, gaze_ext], dim=2) 

        for i,linear in enumerate(self.linears):
            x = linear(x, latent_ext[:,i])
            if self.use_depth and i == self.depth_layers-1:
                depth = self.to_depth(x, None)
                if self.detach_texture and i < self.n_layers-1:
                    x = x.detach()
...
class RenderNet_gaze(nn.Module):
    def __init__(self, min_size, out_size, coarse_size, in_channel, img_dim, seg_dim, style_dim, 
                    channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        super().__init__()
        self.channels = {
        ...
        #self.downs_conv = torch.nn.Conv2d(out_channel, 1, 2, stride=2, padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros')
        self.downs_conv = nn.Sequential(nn.MaxPool2d(2, stride=2), torch.nn.Conv2d(out_channel+3, 1, 1, stride=1, padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros'))
        self.gaze_regressor = nn.Sequential(
            nn.Dropout(p=0.5),
            nn.Linear(out_channel * out_channel, 4096),
    ...
    def forward(self, x, noise=None, randomize_noise=False, skip_rgb=None, skip_seg=None):
        ...
        # predict gaze
        x_ = torch.cat((x,rgb),1)
        x = self.downs_conv(x_) #downsample
        x = x.view(x.shape[0], -1)
        gz = self.gaze_regressor(x)
...
class SemanticGenerator(nn.Module):
    def __init__(self, size=256, style_dim=512, n_mlp=8, channel_multiplier=2,
        ...
        self.gaze_regression = estim_gaze
        ...
        if self.gaze_regression:
            self.render_net = RenderNet_gaze(min_feat_size, size, coarse_size, coarse_channel, 3, seg_dim, style_dim,
    ...
    # def mix_styles_except_gaze(self, styles):
    #     if len(styles) < 2:
    #         # Input is the latent code
    #         if styles[0].ndim < 3:
    #             latent = styles[0].unsqueeze(1).repeat(1, self.n_latent, 1)
    ...
    def forward(self,
        ...
        if self.gaze_regression is False:
            rgb, seg = self.render_net(feat, noise=noise, randomize_noise=randomize_noise, skip_rgb=None, skip_seg=skip_seg)
        else:
            rgb, seg, gz = self.render_net(feat, noise=noise, randomize_noise=randomize_noise, skip_rgb=None, skip_seg=skip_seg)
        ...
        if self.gaze_regression is False:
            ...
            if return_latents:
                return rgb, latent, gz
            elif return_coarse:
                return rgb, seg_coarse, gz
            elif return_all:
                return rgb, seg, seg_coarse, depths, latent, gz
            else:
                return rgb, seg, gz
...
class SemanticGenerator_gaze(nn.Module):
    def __init__(self, size=256, style_dim=512, n_mlp=8, channel_multiplier=2,
                 blur_kernel=[1, 3, 3, 1], lr_mlp=0.01, seg_dim=2,
                 coarse_size=64, min_feat_size=8, local_layers=10, local_channel=64, 
                 coarse_channel=512, base_layers=2, depth_layers=6, residual_refine=True,
        ...
        assert coarse_size < size
        assert min_feat_size < coarse_size and coarse_size % min_feat_size == 0
        self.ID_EYES = 2 # ID defined in labels for eyes
        self.ID_IRIS = 5 # ID defined in labels for iris
        #print("EYES ID ===")
        #print(self.ID_EYES)
        self.size = size
        self.style_dim = style_dim
        ...
        self.n_latent_expand = self.n_local * self.local_layers # Expanded latent space
        print(f"n_latent: {self.n_latent}, n_latent_expand: {self.n_latent_expand}")
        #self.gaze_mlp = False # input gaze before MLP, otherwise add to latent

        self.pos_embed = PositionEmbedding(2, self.local_channel, N_freqs=self.log_size)
        self.local_nets = nn.ModuleList()
        for i in range(0, self.n_local):
            use_depth = i > 0 # disable pseudo-depth for background generator
            if i == self.ID_EYES or self.ID_IRIS == i: # or i == 0:
                self.local_nets.append(LocalGenerator_gaze(local_channel, coarse_channel, local_channel, style_dim, 
                n_layers=local_layers, depth_layers=depth_layers, use_depth=use_depth, detach_texture=detach_texture))
            else:
                self.local_nets.append(LocalGenerator(local_channel, coarse_channel, local_channel, style_dim, 
                n_layers=local_layers, depth_layers=depth_layers, use_depth=use_depth, detach_texture=detach_texture))
    ...
    def forward(self,
        ...
        for i in range(self.n_local):
            x = self.pos_embed(coords[i])
            local_latent = latent[:,i*self.local_layers:(i+1)*self.local_layers]
            if i != self.ID_EYES and i != self.ID_IRIS:
                feat, depth = self.local_nets[i](x, local_latent)
            else:
                feat, depth = self.local_nets[i](x, local_latent, gaze)
            feats.append(feat)
            depths.append(depth)
...
class DualBranchDiscriminator_gaze(nn.Module):
    def __init__(self, img_size, seg_size, img_dim, seg_dim, channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        super().__init__()
        ...
        self.final_conv = ConvLayer(in_channel + 1, self.channels[4], 3)
        self.final_linear = nn.Sequential(
            EqualLinear(self.channels[4] * 4 * 4 + 64, self.channels[4], activation='fused_lrelu'),
            EqualLinear(self.channels[4], 1),
        )
    ...
    def forward(self, img, gaze, seg=None):
        batch = img.shape[0]
        ...
        gaze_ext = self.embed(gaze.to(torch.float32))
        out = torch.cat([out, gaze_ext], dim=1)

        out = self.final_linear(out) # from [b, 8192] to [b, 1]

        return out

### composition tests
#class SemanticCompose_unet(nn.Module):
#class SemanticGenerator_in_efeats(nn.Module):
#class SemanticGenerator_gaze_noise(nn.Module):
#class CompositionDiscriminator(nn.Module):
#class DualBranchDiscriminator_gazeRegressor(nn.Module):
#class DualBranchDiscriminator_gaze_v2(nn.Module):
#class SemanticCompose(nn.Module):
